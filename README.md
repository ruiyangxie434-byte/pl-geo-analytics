...
积分榜来源、快照日期与实时性边界会随 API 返回，并在页面展示。球场坐标用于地图定位，遵循 OpenStreetMap 署名要求。详细口径见 [`docs/DATA_SOURCES.md`](docs/DATA_SOURCES.md)。
